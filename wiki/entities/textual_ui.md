...
- **Независимый ввод** сообщений, очередь сообщений, остановка по ESC. Во время
  стрима `Esc` ставит `_stop_requested`, **убивает запущенные процессы** (через
  `core/process_control.py`, включая PTY-сессии shell) и логирует «Остановлено»
  один раз; в покое — очищает ввод. Раньше `Composer` перехватывал `Esc` всегда
  (гасил событие), из-за чего остановка не срабатывала, пока фокус в композере.

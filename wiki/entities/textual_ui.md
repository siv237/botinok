...
- **Независимый ввод** сообщений, очередь сообщений, остановка по ESC. Во время
  стрима `Esc` ставит `_stop_requested` (генерация прерывается); в покое —
  очищает ввод. Раньше `Composer` перехватывал `Esc` всегда (гасил событие),
  из-за чего остановка не срабатывала, пока фокус в композере.

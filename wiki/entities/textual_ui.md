...
- **Независимый ввод** сообщений, очередь сообщений, остановка по ESC. Во время
...
- **Контракт Esc**: прервать текущее действие и ход → **остановиться и ждать
  следующее сообщение**. Никаких авто-продолжений и автоотправки очереди;
  накопленный ввод возвращается в поле (`flush_tool_buffer` при `_stop_requested`).

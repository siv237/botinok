...
def signal_stop() -> None:
    """Мягкая остановка: только выставить флаг.

    Запущенные через `run()` процессы сами увидят флаг и аккуратно завершат
    свою группу (`terminate`), а ход диалога успеет штатно финализироваться —
    сессия не рвётся. НЕ вызывает `kill_all`: убивать чужие процессы из
    обработчика клавиши опасно (гонка с записью session-файла).
    """
    _stop.set()


def request_stop() -> None:
    """Жёсткая остановка: флаг + немедленное убийство всех процессов."""
    _stop.set()
    kill_all()

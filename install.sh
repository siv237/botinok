...
install_dependencies() {
    ...
    if [ "$OS_NAME" = "Darwin" ]; then
        ...
        if [ "$EUID" -eq 0 ]; then
            ...
            if [ -n "$BREW_USER" ] && [ "$BREW_USER" != "root" ]; then
                ...
                sudo -u "$BREW_USER" brew install python3 lynx git curl jq aria2 chafa
            else
            ...
            brew update
            brew install python3 lynx git curl jq aria2 chafa
        fi
        return 0
    fi
    ...
    if command -v apt-get >/dev/null 2>&1; then
        apt-get update -y
        apt-get install -y python3 python3-venv python3-pip lynx curl git ca-certificates jq aria2 file chafa
    elif command -v dnf >/dev/null 2>&1; then
        dnf install -y python3 python3-virtualenv python3-pip lynx curl git ca-certificates jq aria2 file chafa
    elif command -v yum >/dev/null 2>&1; then
        yum install -y python3 python3-virtualenv python3-pip lynx curl git ca-certificates jq aria2 file chafa
    else
